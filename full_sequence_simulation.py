...
def simulate_repertoires(pos, neg, num_of_repertoires: int, repertoire_size: int,
        ...
        if x < num_of_repertoires / 2:
            ...
            for i in range(int(repertoire_size)):
                if i < num_of_pos_in_pos_repertoires:
                    ...
                    new_rep.append(neg[random.randint(0, len(neg) - 1)])
        else:
...
def main(positive_data_file_name: str, negative_data_file_name: str, num_of_repertoires: int,
         repertoire_size: int,
    ...
    with open(positive_data_file_name) as file:
        tsv_file = csv.reader(file, delimiter="\t")
        i = tsv_file.__next__().index("CDR3")
        for line in tsv_file:
            pos_seqs.append(line[i])
    file.close()
    ...
    reps, disease_list = simulate_repertoires(pos_seqs, neg_seqs, num_of_repertoires,
...
if __name__ == '__main__':
    ...
    main(positive_data_file_name, negative_data_file_name, num_of_repertoires, repertoire_size,
