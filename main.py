...
def simulate_motif_in_repertoires(pos, neg, motif: str, num_of_repertoires: int, repertoire_size: int,
                                  percentage_pos_in_pos_reps: float):
...
def create_repertoire(sequence_objects, export_path, repertoire_id):
    repertoire = Repertoire.build_from_sequence_objects(sequence_objects=sequence_objects, path=export_path, filename_base=repertoire_id,
...
def main():
    ...
    neg_seqs = []
    with open("SimData/many_seqs.tsv") as file:
    ...
    pos_seqs = []
    with open("SimData/few_seqs.tsv") as file:
        tsv_file = csv.reader(file, delimiter="\t")
        for line in tsv_file:
            pos_seqs.append(line[0])
    file.close()

    filtered = filter_out_motif(neg_seqs, given_motif)

    print(len(filtered), " / ", len(neg_seqs))
    # return

    export_path = PathlibPath(os.getcwd()) / "airr_exporter_repertoire/"
    ...
    reps, disease_list = simulate_motif_in_repertoires(pos_seqs, neg_seqs, given_motif, 100, 1000, 0.5)
    # reps, disease_list = simulate_motif_in_repertoires(pos_seqs, neg_seqs, given_motif, 2, 100, 0.5)
